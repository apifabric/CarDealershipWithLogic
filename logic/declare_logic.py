...
def declare_logic():
    ...
    # Counts the number of unpaid sales associated with a customer
    Rule.count(derive=Customer.unpaid_sales_count, as_count_of=Sale, where=lambda row: not row.is_paid)

    # Sum of car stock inventory should not exceed dealer's limits
    Rule.sum(derive=Dealer.inventory_total, as_sum_of=Inventory.stock_amount)

    # Ensure sum of stock inventory does not exceed dealer's limits
    Rule.constraint(validate=Dealer, as_condition=lambda row: row.inventory_total <= row.stock_limit, error_msg="Dealer's inventory exceeds stock limit")
